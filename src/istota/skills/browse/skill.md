...
istota-skill browse extract --session <id> -s ".price"
...
1. Fetch the hub/index page with `--keep-session`:
   ```bash
   istota-skill browse get "https://apnews.com/hub/world-news" --keep-session
   ```
...
3. Navigate to each article using `get --session` with the full URL:
   ```bash
   istota-skill browse get "https://apnews.com/article/abc123" --session <session_id>
   ```
...
1. Fetch the hub page with `--keep-session`:
   ```bash
   istota-skill browse get "https://www.theguardian.com/world" --keep-session
   ```
2. If the `links` array has no article links, use `extract` with a site-specific selector:
   ```bash
   istota-skill browse extract --session <id> -s "a[data-link-name='article']"
   ```
