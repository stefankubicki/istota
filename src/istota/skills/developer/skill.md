...
$DEVELOPER_AUTHOR_CREDIT}"
   ...
   Closes #123${CREDIT}"
   ```
4. **Commit style**: Never include LLM/AI attribution in commit messages (no `Generated by` or similar). If `$DEVELOPER_AUTHOR_CREDIT` is set, append it after a blank line at the end of every commit message. Otherwise, write commits without any trailer.
