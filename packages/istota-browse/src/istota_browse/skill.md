...
```bash
# Fetch a page
python -m istota_browse get "https://example.com"
python -m istota_browse get "https://example.com" --keep-session --timeout 60
python -m istota_browse get "https://example.com" --wait-for "article.content"

# Navigate within an existing session (preserves cookies, referrer, state)
python -m istota_browse get "https://example.com/page2" --session <id>

# Fetch only links (no page text)
python -m istota_browse links "https://example.com"
python -m istota_browse links "https://example.com" --selector "nav a"

# Screenshot
python -m istota_browse screenshot "https://example.com" -o /tmp/page.png
python -m istota_browse screenshot --session <id> -o /tmp/page.png --full-page

# Extract by CSS selector
python -m istota_browse extract "https://example.com" -s "article"
python -m istota_browse extract --session <id> -s ".price"

# Interact with existing session (click, fill forms, scroll)
python -m istota_browse interact <id> --click ".button"
python -m istota_browse interact <id> --fill "#email=user@example.com"
python -m istota_browse interact <id> --scroll down --scroll-amount 1000

# Close session
python -m istota_browse close <id>
```
...
1. Fetch the hub/index page with `--keep-session`:
   ```bash
   python -m istota_browse get "https://apnews.com/hub/world-news" --keep-session
   ```
...
3. Navigate to each article using `get --session` with the full URL:
   ```bash
   python -m istota_browse get "https://apnews.com/article/abc123" --session <session_id>
   ```
...
5. Close the session when done:
   ```bash
   python -m istota_browse close <session_id>
   ```
...
1. Fetch the hub page with `--keep-session`:
   ```bash
   python -m istota_browse get "https://www.theguardian.com/world" --keep-session
   ```
2. If the `links` array has no article links, use `extract` with a site-specific selector:
   ```bash
   python -m istota_browse extract --session <id> -s "a[data-link-name='article']"
   ```
...
```bash
python -m istota_browse links "https://www.theguardian.com/world" --selector "a[data-link-name='article']"
```
...
```bash
python -m istota_browse interact <session_id> --scroll down --scroll-amount 2000
```
**Max 3 scroll rounds** — stop and use what you have.

## Rules

**Run browse commands yourself.** Always execute `python -m istota_browse` directly in Bash. Never delegate browsing to a subtask or subagent — they lose the session context and skill instructions, leading to repeated failures.
